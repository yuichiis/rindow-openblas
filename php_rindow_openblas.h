...
# define PHP_RINDOW_OPENBLAS_VERSION "0.1.7"
...
static inline void *php_rindow_openblas_get_address(
    php_rindow_openblas_buffer_t* buffer,zend_long offset,int valueSize)
{
    return (uint8_t *)(buffer->data)+(offset*valueSize);
}


extern int php_rindow_openblas_dtype_to_valuesize(zend_long dtype);
extern int php_rindow_openblas_dtype_is_int(zend_long dtype);
extern int php_rindow_openblas_dtype_is_float(zend_long dtype);
extern int php_rindow_openblas_dtype_is_bool(zend_long dtype);
